locals {
  ...
  controlplane_name = replace(substr("${var.name}-rke2-cp", 0, 31), "/-$/", "")
  server_name       = replace(substr("${var.name}-rke2-server", 0, 31), "/-$/", "")
  supervisor_name   = replace(substr("${var.name}-rke2-supervisor", 0, 31), "/-$/", "")
}
